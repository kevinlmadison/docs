...
const pool = new Pool({
  host: "postgresql.default.svc.dev.ahq",
  port: 5432,
  database: "mydatabase",
  user: "myuser",
  ...
  const sendComments = async () => {
    try {
      const { rows } = await pool.query(
        "SELECT * FROM book_reviews ORDER BY post_time DESC;",
      );
...
app.post("/insert", async (req, res) => {
  try {
    ...
    await pool.query(
      "INSERT INTO book_reviews (post_time,content, sentiment) VALUES ($1, $2, $3)",
      [postTime, reviewText, sentimentResult],
    );
...
app.post("/add", async (req, res) => {
  try {
    ...
    if (receivedEvent.type === "new-review-comment") {
      ...
      if (!response.ok) {
        // If the response status code is not 2xx, consider it a failure
      ...
      return res
        .status(200)
        .json({ success: true, message: "Event forwarded successfully" });
    } else {
